    ...
    for paddle_name in only_in_paddle:
        if paddle_name.endswith("._mean"):
            ...
            if expected_torch in torch_buffer_names:
                transformations["._mean -> .running_mean"].append(
                    (paddle_name, expected_torch)
                )
        elif paddle_name.endswith("._variance"):
            expected_torch = paddle_name.replace("._variance", ".running_var")
            if expected_torch in torch_buffer_names:
                transformations["._variance -> .running_var"].append(
                    (paddle_name, expected_torch)
                )
        else:
...
def save_full_comparison(
    paddle_params, paddle_buffers, torch_params, torch_buffers, output_file
):
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"PaddlePaddle: {len(paddle_params)} parameters, {len(paddle_buffers)} buffers\n"
        )
        f.write(
            f"PyTorch: {len(torch_params)} parameters, {len(torch_buffers)} buffers\n\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--torch-config", type=str, required=True, help="Path to PyTorch config file"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="logs/model_params_comparison.txt",
        help="Output file for full comparison (default: logs/model_params_comparison.txt)",
    )
    parser.add_argument(
        "--num-lines",
        type=int,
    ...
    try:
        ...
        save_full_comparison(
            paddle_params, paddle_buffers, torch_params, torch_buffers, args.output
        )
