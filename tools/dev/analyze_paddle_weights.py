...
def build_paddle_model(config_path: str):
    ...
    if importlib.util.find_spec("paddle") is None:
        logger.error("PaddlePaddle is not installed. Run: uv sync --extra dev")
        return None
...
def print_model_summary(paddle_stats, pytorch_stats, model_name):
    ...
    # Total
    print(
        f"{'TOTAL':<20} {paddle_total:>20,} {pytorch_total:>20,} {diff:>+20,} ({diff_pct:>+6.2f}%)"
    )
    print("-" * 85)

    # Components
    all_components = set(paddle_stats["components"].keys()) | set(
        pytorch_stats["components"].keys()
    )
    for comp in sorted(all_components):
        ...
        print(
            f"{comp:<20} {paddle_comp:>20,} {pytorch_comp:>20,} {comp_diff:>+20,} ({comp_diff_pct:>+6.2f}%) {status}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare RT-DETRv3 model architectures"
    )
    parser.add_argument(
        "--paddle-config",
        default=str(
    ...
    parser.add_argument(
        "--torch-config",
        default=str(project_root / "configs/rtdetrv3/rtdetrv3_r18vd_6x_coco.yml"),
