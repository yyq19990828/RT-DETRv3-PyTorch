...
def get_basic_sample():
    ...
    return {
        ...
        "gt_bbox": np.array(
            [[100, 100, 200, 200], [300, 300, 400, 400]], dtype=np.float32
        ),
...
OPERATOR_CONFIGS = {
    ...
    "RandomShortSideResize": {
        "short_side_sizes": [480, 512, 544, 576, 608, 640],
        "max_size": 640,
    },
...
class TestRegisteredOperators:
    ...
    def test_all_configured_operators_instantiation(self):
        ...
        # Print summary
        print(f"\n{'=' * 60}")
        print("Operator Instantiation Summary:")
    ...
    def test_operator_call_basic(self):
        ...
        for op_name in test_ops:
            if (
                op_name not in self.get_all_operators()
                or op_name not in OPERATOR_CONFIGS
            ):
                continue
...
class TestOperatorCategories:
    ...
    def test_resize_operators(self):
        ...
        for op_name in resize_ops:
            ...
            assert result["image"].shape[2] == 3, (
                f"{op_name}: should preserve color channels"
            )
    ...
    def test_augmentation_operators(self):
        ...
        for op_name in aug_ops:
            ...
            assert "image" in result
            assert (
                result["image"].dtype == sample["image"].dtype
