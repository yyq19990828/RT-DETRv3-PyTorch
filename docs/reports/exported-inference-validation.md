...
**不作声明**：本报告只覆盖 R18、当前 Python Infer CLI、CPU/FP32 和固定 640。它不证明 ONNX Runtime CUDA、TorchScript GPU、动态高宽、TensorRT、C++、量化或外部客户端预处理等价；R34/R50 已有 tensor 级导出证据，但尚未重复本轮用户侧三后端命令。
...
- checkpoint 保持既有设备默认值并可选 EMA；ONNX/TorchScript 默认且只接受 CPU，任何 `--use-ema` 或显式非 CPU 组合在模型加载前失败。
