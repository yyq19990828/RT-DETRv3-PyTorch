...
新增 engine 的 4 个活跃回归后，显式设置 `CUDA_VISIBLE_DEVICES=''` 的本地 `.venv` CPU 实测为 `230 passed, 5 skipped, 34 deselected`，全包 `5,945/13,264`（`44.82%`），直接维护范围仍为 `1,202/1,797`（`66.89%`）。`engine` 新增 52 条实现语句，覆盖语句净增 63 条；全包尚未达到 45%，因此本批不提高 44% 下限。当前提交的托管复验尚待 CI 完成。
