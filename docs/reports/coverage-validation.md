...
`scripts/check_coverage.py` 将 coverage data 和 JSON 报告写入临时目录，命令结束时自动清理。最近一次托管 `test` extra 的结果为 `221 passed, 7 skipped, 17 deselected, 6 warnings in 12.27s`；17 项因 `paddle` marker 被排除，7 项因当前环境缺少所需的 Paddle/CUDA 能力跳过。`tests/legacy/` 由 Pytest 配置明确排除，但 `src/ppdet_pytorch/` 内没有源文件被从全包统计中删除。
...
新增 utils 活跃测试后，显式设置 `CUDA_VISIBLE_DEVICES=''` 的本地 `.venv` CPU 实测为 `225 passed, 5 skipped, 34 deselected`，全包 `5,853/13,203`（`44.33%`），直接维护范围 `1,173/1,788`（`65.60%`）。未隐藏 GPU 时额外执行 5 个 CUDA 可用用例，得到 `44.96%`；该设备差异不用于 CPU 门槛。下表已更新为本地 CPU 结果，托管复验在实现提交后记录。
