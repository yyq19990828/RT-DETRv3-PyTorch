...
| `utils` | 751 | 323 | 43.0% |
| **全包** | **13,557** | **6,790** | **50.08%** |

直接维护范围指 M1–M5 首批质量门禁中的 `cli`、`conversion`、`core` 和 `deploy`，共 `2,011` 条语句，覆盖 `1,708` 条，覆盖率为 **84.93%**。这个子集用于屏蔽回退，不代表其他模块不维护，也不是对整个“已迁移核心”的最终定义。

## 门禁与限制

当前可执行门禁同时要求：

- 非 Paddle 全包语句覆盖率不低于 **50%**。
- `cli/conversion/core/deploy` 合计覆盖率不低于 **84%**。

两个阈值都低于当前实测值，是防止覆盖率回退的初始下限，不是完成目标。`ROADMAP.md` 中的 90% 目标仍未完成；不应通过排除 `data`、`metrics` 或其他低覆盖源文件来声称全包达标。

下一轮优先覆盖仍低于直接维护范围平均值的用户路径：`cli/export.py` 和 `cli/convert.py`。之后再以实际新增测试证据逐步提高门禁。
