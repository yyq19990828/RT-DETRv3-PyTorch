...
6. 合并显式构造参数，它们的优先级高于配置块和 `from_config()` 返回值，然后调用构造函数。

**已验证（2026-07-18）**：命名配置块、显式 kwargs 优先级、`from_config()` 上下文隔离，以及空的显式 `merge_config` 目标都有活跃单元测试。这只证明当前合同，不代表已复制 PaddleDetection 的全部冲突处理语义。
