...
当前官方 R18/R34/R50 已覆盖文件、映射、参数、eval 模块、后处理、受控训练 loss 和整体梯度方向。R18 还通过了同权重完整 val2017 指标和单图可视化门禁；R34/R50 的同等 COCO 对照、三变体真实训练收敛和跨框架优化器状态迁移仍未验证。
...
R18 已按该协议在 COCO `000000000139.jpg` 上验证：`score >= 0.3` 的两侧 `30` 个预测全部匹配，最大 score 差 `1.37e-6`，最大框坐标差 `9.16e-5 px`。统一渲染图、JSON 和命令见[R18 预测可视化报告](../reports/prediction-visualization.md)。单图一致不能代替完整 val2017 AP 或训练收敛证据。
...
- R34/R50 的真实 COCO 预处理输入与 AP 对照，以及三个变体的完整训练收敛。
