...
class PPYOLOEHead(nn.Module):
    ...
    def _init_weights(self):
        ...
        proj = torch.linspace(
            self.reg_range[0], self.reg_range[1] - 1, self.reg_channels
        ).reshape(1, self.reg_channels, 1, 1)
    ...
    def _generate_anchors(self, feats=None, dtype=torch.float32):
        ...
        for i, stride in enumerate(self.fpn_strides):
            ...
            shift_x = (
                torch.arange(end=w, dtype=dtype, device=device) + self.grid_cell_offset
            )
            shift_y = (
                torch.arange(end=h, dtype=dtype, device=device) + self.grid_cell_offset
            )
    ...
    def forward_eval(self, feats):
        ...
        for i, feat in enumerate(feats):
            ...
            reg_dist = reg_dist.reshape(
                -1, 4, self.reg_channels, num_locations
            ).permute(0, 2, 3, 1)
            ...
            cls_score_list.append(
                cls_score.reshape(-1, self.num_classes, num_locations)
            )
    ...
    def _bbox_decode(self, anchor_points, pred_dist):
        ...
        _, num_locations, _ = pred_dist.shape
        pred_dist = F.softmax(
            pred_dist.reshape(-1, num_locations, 4, self.reg_channels), dim=-1
        )
    ...
    def _bbox2distance(self, points, bbox):
        ...
        lt = points - x1y1
        rb = x2y2 - points
        return torch.cat([lt, rb], -1).clamp(
            self.reg_range[0], self.reg_range[1] - 1 - 0.01
        )
    ...
    def _df_loss(self, pred_dist, target, lower_bound=0):
        ...
        loss_left = (
            F.cross_entropy(
                pred_dist, (target_left - lower_bound).reshape(-1), reduction="none"
            ).reshape_as(target)
            * weight_left
        )
        loss_right = (
            F.cross_entropy(
                pred_dist, (target_right - lower_bound).reshape(-1), reduction="none"
            ).reshape_as(target)
            * weight_right
        )
    ...
    def _bbox_loss(
        ...
        if mask_positive.any():
            ...
            assigned_bboxes_pos = assigned_bboxes.masked_select(bbox_mask).reshape(
                -1, 4
            )
            bbox_weight = (
                assigned_scores.sum(-1).masked_select(mask_positive).unsqueeze(-1)
            )
            ...
            assigned_ltrb_pos = assigned_ltrb.masked_select(bbox_mask).reshape(-1, 4)
            loss_dfl = (
                self._df_loss(
                    pred_dist_pos,
                    assigned_ltrb_pos,
    ...
    def get_loss_from_assign(
        ...
        if self.use_varifocal_loss:
            one_hot_label = F.one_hot(assigned_labels.long(), self.num_classes + 1)[
                ..., :-1
            ]
    ...
    def post_process(self, head_outs, scale_factor):
        ...
        if self.exclude_post_process:
            return (
                torch.cat([pred_bboxes, pred_scores.permute(0, 2, 1)], dim=-1),
                None,
                None,
            )
        else:
