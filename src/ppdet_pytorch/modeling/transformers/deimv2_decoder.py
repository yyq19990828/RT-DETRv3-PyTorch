...
class DEIMv2TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.lqe_layers = nn.ModuleList(
            [
                copy.deepcopy(LQE(4, 64, 2, reg_max, act=act))
    ...
    def forward(
        ...
        for index, layer in enumerate(self.layers):
            ...
            output = layer(
                output, ref_points_input, value, spatial_shapes, attn_mask,
                query_pos_embed,
            )
...
class DEIMTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.reg_scale = nn.Parameter(
            torch.tensor([reg_scale]), requires_grad=False
        )
        ...
        self.dec_score_head = nn.ModuleList(
            [
                (
                    dec_score_head
                    if share_score_head
                    else copy.deepcopy(dec_score_head)
                )
        ...
        self.dec_bbox_head = nn.ModuleList(
            [
                (
                    dec_bbox_head
                    if share_bbox_head
                    else copy.deepcopy(dec_bbox_head)
                )
                for _ in range(self.eval_idx + 1)
            ]
            + [
    ...
    def convert_to_deploy(self):
        self.dec_score_head = nn.ModuleList(
            [nn.Identity()] * (self.eval_idx)
            + [self.dec_score_head[self.eval_idx]]
        )
        self.dec_bbox_head = nn.ModuleList(
            [
                self.dec_bbox_head[index]
    ...
    def _get_encoder_input(self, feats: List[torch.Tensor]):
        proj_feats = [
            self.input_proj[index](feat) for index, feat in enumerate(feats)
        ]
    ...
    def _generate_anchors(
        ...
        valid_mask = (
            (anchor_tensor > self.eps) * (anchor_tensor < 1 - self.eps)
        ).all(-1, keepdim=True)
