...
class ModelOutputValidator:
    ...
    def validate_forward_pass(
        ...
        is_rtdetr = all(
            hasattr(paddle_model, name)
            for name in ('backbone', 'neck', 'transformer')
        )
        with paddle.no_grad():
            if is_rtdetr:
                body_feats = paddle_model.backbone(paddle_input)
                neck_feats = paddle_model.neck(body_feats)
                transformer_output = paddle_model.transformer(
                    neck_feats, None, None
                )
                if (isinstance(transformer_output, (tuple, list)) and
                        len(transformer_output) >= 4):
                    paddle_output = {
