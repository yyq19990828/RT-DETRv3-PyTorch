...
def cutout(image, pad_size, replace=0):
    ...
    image = np.where(
        np.equal(mask, 0), np.ones_like(image, dtype=image.dtype) * replace, image
    )
...
def random_shift_bbox(image, bbox, pixel_scaling, replace, new_min_bbox_coords=None):
    ...
    minval_y = clip_y(min_y - np.int32(pixel_scaling * float(bbox_height) / 2.0))
    maxval_y = clip_y(min_y + np.int32(pixel_scaling * float(bbox_height) / 2.0))
...
def _shear_bbox(bbox, image_height, image_width, level, shear_horizontal):
    ...
    new_coords = np.matmul(translation_matrix, np.transpose(coordinates)).astype(
        np.int32
    )
...
def autocontrast(image):
    ...
    def scale_channel(image):
        ...
        def scale_values(im):
            ...
            np.clip(im, a_min=0, a_max=255.0)
...
def _bbox_cutout_level_to_arg(level, hparams):
    cutout_pad_fraction = (level / _MAX_LEVEL) * 0.75  # hparams.cutout_max_pad_fraction
...
def level_to_arg(hparams):
    return {
        ...
        "Cutout_Only_BBoxes": lambda level: (
            int((level / _MAX_LEVEL) * 50),
        ),  # hparams.cutout_bbox_const
        # pylint:enable=g-long-lambda
    }
