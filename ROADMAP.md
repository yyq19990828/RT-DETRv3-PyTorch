...
**执行计划**：[`M6——性能、质量与发布计划`](docs/plans/2026-07-19-m6-performance-quality-release.md)。2026-07-19 初始快照为全包 45% 语句覆盖率、128 个待 Ruff 格式化文件、293 项默认 Ruff lint 和 123 项 Mypy 全包错误。当前 Ruff 已覆盖全部活跃 Python 文件，干净 `test` extra 的非 Paddle 全包覆盖率实测为 42.48%，已建立 42%/65% 双回退下限；完整 Mypy、90% 覆盖率目标、CUDA、性能和发布验收仍未完成。
