...
**执行计划**：[`M6——性能、质量与发布计划`](docs/plans/2026-07-19-m6-performance-quality-release.md)。2026-07-19 初始快照为全包 45% 语句覆盖率、128 个待 Ruff 格式化文件、293 项默认 Ruff lint 和 123 项 Mypy 全包错误。当前 Ruff/Mypy 已扩展到全部活跃 Python 范围和纳入门禁的仓库脚本；隐藏 GPU 的本机非 Paddle CPU 覆盖率为全包 50.98%、直接维护范围 90.80%，回退下限已提高到 50.5%/90%，直接维护范围的 90% 目标已有本地证据。Python 3.9–3.12 CPU CI、本机 CUDA 运行和 R18 同机 CPU/CUDA model-only 性能证据均已通过。PyTorch 四个 workload 吞吐均高于 Paddle，CUDA 训练峰值 allocated 显存约高 16%，按维护者决策不追求训练优化完全对齐。发布候选的许可、清单、wheel/sdist、包外安装和模型 checksum 已验证；三个检测权重与 R18-vd backbone 初始化权重已纳入统一发布合同，R18/R34/R50 的 Paddle 原权重/PyTorch 转换权重均完成 COCO 同图统一渲染和机器可读差异报告。详见[性能报告](docs/reports/performance-validation.md)、[发布候选报告](docs/reports/release-validation.md)和[预测可视化报告](docs/reports/prediction-visualization.md)。可选 end-to-end DataLoader/profile、权重公开发布和从公开 URL 回读尚未完成。
